        ...
        with PiCamera() as camera:
            ...
            for frame in camera.capture_continuous(stream, format="jpeg", use_video_port=True):
                if self.send_inst:
                    ...
                    img = cv2.imdecode(np.fromstring(frame.getvalue(), dtype=np.uint8), cv2.CV_LOAD_IMAGE_GRAYSCALE)

                    roi = img[120:240, :]
                    # success_write = cv2.imwrite('training_images/frame_num{:>05}.jpg'.format(frame_num), img)
                    cv2.imwrite('training_images/frame_num{:>05}.jpg'.format(frame_num), img)
